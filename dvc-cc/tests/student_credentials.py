...
child = pexpect.spawn('dvc-cc init --htw-student')

child.expect([pexpect.TIMEOUT, ".*matriculation number.*"])
child.sendline(args.dvcusername)
print('SET matriculation number')
...
child = pexpect.spawn('dvc-cc keyring --htw-student --set --keyring-service '+args.keyring_service)

child.expect([pexpect.TIMEOUT, ".*password for the DT-Cluster.*"])
child.sendline(args.dvcpassword)
print('SET matriculation number')
